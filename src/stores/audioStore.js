...
export const useAudioStore = defineStore('audio', () => {
  ...
  async function convertFile(fileData) {
    ...
    // Send explicit bitrate for MP3/AAC to ensure 320 kbps at max quality
    const bitrates = {
    ...
    if (currentFormat.value === 'mp3' || currentFormat.value === 'aac' || currentFormat.value === 'm4a') {
      ...
      // Backend expects format "320k", not just "320"
      formData.append('bitrate', `${targetBitrate}k`)
    }
    ...
    try {
      ...
      console.log('📊 Quality:', currentQuality.value, 'Format:', currentFormat.value, 'Bitrate:', formData.get('bitrate') || 'default')
